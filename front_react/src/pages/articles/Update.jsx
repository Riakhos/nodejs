...
const UpdateArticle = () => {
	...
	useEffect(() => {
		const fetchArticleById = async () => {
			try {
				const { data } = await axios.get(`http://localhost:8000/api/article/update/${id}`)
	...
	const handleSubmit = async (event) => {
		...
		try {
			const response = await axios.put(`http://localhost:8000/api/article/update/${id}`, formPayload, {
